...
-- Compatible with an existing LearnTense database where tenses/questions may already exist.
...
-- IMPORTANT: upgrade an existing questions table instead of assuming it is empty.
...
update public.questions set difficulty = 'easy' where difficulty is null;
update public.questions set created_at = now() where created_at is null;

-- Add the tense foreign key only if it does not already exist.
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'questions_tense_id_fkey') then
    alter table public.questions add constraint questions_tense_id_fkey
      foreign key (tense_id) references public.tenses(id) on delete cascade;
  end if;
end $$;
...
-- Starter questions. Existing rows are preserved.
